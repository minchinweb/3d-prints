...
// translate([_x02 + frame_thickness, 0, 0])
eye_hook();
// marble_tank();
// seeding_wheel();
// track_plate();
...
module exit_tube() {
    ...
    difference() {
        ...
        translate([exit_pipe_outer_diameter * 2, 0, 5])
        rotate([180, 0, 0])
...
module frame() {
    ...
    translate([-frame_back_length + _x02, 0, 0]) {
        ...
        // motor box (2/2)
        translate([_x06 - _x07, _y01 + frame_thickness, 0]) {
            _frame_wheel_support(_frame_wheel_drop);
...
module assembly_view(exploded = 0) {
    ...
    color("salmon")
    translate([_x03, _y05, _z03 + 3 * _z_explode])
    seeding_wheel();
    ...
    color("OrangeRed")
    translate([0, 0, _z01 - frame_thickness + 2 * _z_explode])
    upper_frame();

    color("Yellow")
    // translate([-marble_diameter * 11 + marble_tolerance * 11 - seeding_wheel_diameter / 2, 0, 44])
    translate([_x03, _y05, _z02 + frame_rotating_clearance + 3 * _z_explode])
    rotate([0, 0, 201])
    translate([-seeding_radius, 0, 0])
    marble_tank();
